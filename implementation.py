...
def Empty():
    class MatrixMultiplierGUI:

        def __init__(self, master):
            self.master = master
            master.title("Matrix Multiplier")

            # Create matrix A label and input boxes
            tk.Label(master, text="Matrix A").grid(row=0, column=0, padx=(20, 10), pady=(20, 5))
            self.a_entries = []
            for i in range(3):
                row = []
                for j in range(3):
                    entry = tk.Entry(master, width=8, font=('Arial', 14))
                    entry.grid(row=i+1, column=j, padx=10, pady=5)
                    row.append(entry)
                self.a_entries.append(row)

            # Create matrix B label and input boxes
            tk.Label(master, text="Matrix B").grid(row=0, column=4, padx=(10, 20), pady=(20, 5))
            self.b_entries = []
            for i in range(3):
                row = []
                for j in range(3):
                    entry = tk.Entry(master, width=8, font=('Arial', 14))
                    entry.grid(row=i+1, column=j+4, padx=10, pady=5)
                    row.append(entry)
                self.b_entries.append(row)

            # Create button to perform matrix multiplication
            tk.Button(master, text="Multiply", command=self.multiply_matrices, font=('Arial', 16)).grid(row=4, column=2, pady=(20, 10))

            # Create label to display the result
            self.result_label = tk.Label(master, text="", font=('Arial', 14))
            self.result_label.grid(row=5, column=0, columnspan=7, padx=20, pady=(10, 20))

        def multiply_matrices(self):
            # Get matrix A values from input boxes
            a = np.zeros((3, 3))
            for i in range(3):
                for j in range(3):
                    a[i][j] = float(self.a_entries[i][j].get())

            # Get matrix B values from input boxes
            b = np.zeros((3, 3))
            for i in range(3):
                for j in range(3):
                    b[i][j] = float(self.b_entries[i][j].get())
...
filemenu.add_command(label="Matrix", command=Empty)
